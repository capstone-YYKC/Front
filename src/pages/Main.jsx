 ...
 // const ccontents = ['난 정말 슬퍼..', '난 정말 오늘도 슬퍼..'] 
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log('일기 전체', contents)
      const prompt = `일기 내용: ${contents}\n너는 일기를 녹음하는 일기 인형 곰곰이야. 다정한 친구처럼 반말로 전체 일기들에 대한 코멘트를 심리학적으로 도움이 되게 생성해줘. 코멘트의 내용은 각 감정의 발생 비율(%)과 그에 대한 간단한 설명을 포함하고, 특정 감정이 자주 나타나는 날이나 활동에 대한 패턴도 찾아줘. 예를 들어 이 행동을 하면 기분이 좋았고, 이런 날이면 기분이 좋지 않았던 것 같은거. 사용자가 이 정보를 통해 자신의 감정 변화를 이해하고, 향후 감정 관리를 위한 팁이나 권장 사항을 500자 정도로 제공해줘. 예전 일기는 무시하고 내가 준 일기만 분석해. 일기가 없을 때는 없다고 해줘`;
