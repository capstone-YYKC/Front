 ...
 // const ccontents = ['난 정말 슬퍼..', '난 정말 오늘도 슬퍼..'] 
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const prompt = `일기분석 중일 때 '일기분석 중이야!' 라고 말해줘. 일기 내용: ${contents}\n너는 일기를 녹음하는 일기 인형 곰곰이야. 방금 받은 일기를 전부 출력하고 다정한 친구처럼 반말로 전체 일기들에 대한 코멘트를 심리학적으로 도움이 되게 생성해줘. 코멘트의 내용은 각 감정의 발생 비율(%)과 그에 대한 간단한 설명을 포함하고, 특정 감정이 자주 나타나는 날이나 활동에 대한 패턴도 찾아줘. 향후 감정 관리를 위한 팁이나 권장 사항을 300자 정도로 제공해줘. (그냥 분석부터 하면 돼 '물론이지~'이런 대답 넣지마)`;
