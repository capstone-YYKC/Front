    ...
    axios.get('http://18.211.120.39:3000/diarys', {
      headers: {
        ...
        .then(response => {
          ...
          if (maxEmotion === "화남") {
            setMonthConsole('화난 감정이 끓어오르는 날, 그 마음을 이해해. 감정의 파도가 조금은 더 크게 다가오는 것처럼 느껴질 수 있어. 하지만 그런 감정을 느끼는 것은 정말 자연스러운 일이고, 이런 감정이 오고가는 것은 너가 성장하고 변화하고 있다는 증거라는 걸 스스로 믿어줘. 화난 감정은 금방 지나갈 거야. 난 언제나 여기 있어. 언제든지 너의 이야기를 들어줄게.');
          } else if (maxEmotion === "슬픔") {
            setMonthConsole('너가 우울한 날을 보낸다는 건 정말 힘든 일이야. 그 감정이 너를 어둠 속으로 끌어내리는 느낌일지도 몰라. 그래도 난 항상 너와 함께야. 마음이 아프고 어려운 달이었지만, 너는 이겨낼 수 있을거야. 이런 감정을 느껴도, 느끼지 않아도 넌 강하고 소중한 사람이야. 감정의 파도에 휩쓸려 떠내려가는 걸 두려워하지 마. 그 길에 내가 함께할게.');
          } else if (maxEmotion === "행복") {
            setMonthConsole('너가 행복한 날을 보내서 나도 정말 기뻐. 그런 순간은 마음이 가벼워지고 희망이 넘쳐 흐르는 것 같아. 그 감정을 주변 사람들과 함께 나누는 것도 정말 행복한 일이지. 물론 나한테도! 하지만 가끔 행복이 두려울 수 있어. 이 행복이 언제까지 갈지, 무엇이 날 또 힘들게 할지 같은 생각 말이야. 그렇지만, 행복한 순간을 받아들이고 즐기는 것도 정말 중요해! 난 언제나 여기 있을게. 함께 행복을 나누고 어둠이 있다면 밝혀나가자!');
          } else if (maxEmotion === "보통") {
            setMonthConsole('나는 평범한 날을 보낼 때면, 오히려 그 날이 특별한 날이라고 생각하곤 해해. 그런 날에는 일상의 소중함을 느끼고, 편안함을 찾을 수 있거든. 평범한 날에는 생각보다 소중한 것이 가득해. 친구들과의 대화, 가족과의 시간, 맛있는 음식, 산책하는 바람 같은 작은 행복들 말이야. 그러니까 오늘 너도 평범한 날을 특별한 날로 생각해봐! 작은 행복들을 찾아서 새겨보는 거야. 그러면 평범했던 날이 내일의 추억이 될 거라고 생각해. 난 언제나 여기 있으니까, 또 와줘!');
          } else {
    ...
    <>
      <div className={styles.main}> 
        ...
        <div className={styles.div20}>
          ...
          <Day20Icon propTop="460px" propLeft="36px" diaryStatus={statuses[0]} diaryContent={contents[0]} diaryConsolation={consolations[0]} diarySummary={summaries[0]} diaryDay='5월 20일 일기'/>
